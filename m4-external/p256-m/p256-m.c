...
static uint32_t point_check(const uint32_t x[8], const uint32_t y[8]) {
    ...
    for (unsigned i = 0; i < 3; i++) m256_sub_p(rhs, rhs, x); /* x^3 - 3x */
 ...
 */
static void point_add_or_double_leaky(uint32_t x3[8], uint32_t y3[8], const uint32_t x1[8], const uint32_t y1[8],
 ...
 */
static void scalar_mult(uint32_t rx[8], uint32_t ry[8], const uint32_t px[8], const uint32_t py[8],
                        const uint32_t s[8]) {
    /*
...
static void ecdsa_m256_from_hash(uint32_t z[8], const uint8_t *h, size_t hlen) {
    ...
    if (hlen < 32) {
        uint8_t p[32];
        for (unsigned i = 0; i < 32; i++) p[i] = 0;
        for (unsigned i = 0; i < hlen; i++) p[32 - hlen + i] = h[i];
        u256_from_bytes(z, p);
    }
    else {
        u256_from_bytes(z, h);
