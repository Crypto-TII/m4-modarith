...
static void modmul_PM383_ct(const spint *a, const spint *b, spint *c) {
    ...
    spint s;
    spint mask = ((spint)1 << 28u) - (spint)1;
    tt = (dpint)a[1] * (dpint)b[13];
    tt += (dpint)a[2] * (dpint)b[12];
    tt += (dpint)a[3] * (dpint)b[11];
    tt += (dpint)a[4] * (dpint)b[10];
    tt += (dpint)a[5] * (dpint)b[9];
    tt += (dpint)a[6] * (dpint)b[8];
    tt += (dpint)a[7] * (dpint)b[7];
    tt += (dpint)a[8] * (dpint)b[6];
    tt += (dpint)a[9] * (dpint)b[5];
    tt += (dpint)a[10] * (dpint)b[4];
    tt += (dpint)a[11] * (dpint)b[3];
    tt += (dpint)a[12] * (dpint)b[2];
    tt += (dpint)a[13] * (dpint)b[1];
    lo = (spint)tt & mask;
    t += (dpint)lo * (dpint)0x17600;
    hi = (spint)(tt >> 28u);
    ...
    lo = (spint)tt & mask;
    t += (dpint)(spint)(lo + hi) * (dpint)0x17600;
    hi = (spint)(tt >> 28u);
...
static void modsqr_PM383_ct(const spint *a, spint *c) {
    ...
    spint lo;
    spint hi;
    tt = (udpint)a[1] * (udpint)a[13];
    tt += (udpint)a[2] * (udpint)a[12];
    tt += (udpint)a[3] * (udpint)a[11];
    tt += (udpint)a[4] * (udpint)a[10];
    tt += (udpint)a[5] * (udpint)a[9];
    tt += (udpint)a[6] * (udpint)a[8];
    tt *= 2;
    tt += (udpint)a[7] * (udpint)a[7];
    lo = (spint)tt & mask;
    t2 = (udpint)a[0] * (udpint)a[0];
    t2 += (udpint)lo * (udpint)0x17600;
    hi = (spint)(tt >> 28u);
    t += t2;
    spint v0 = (spint)t & mask;
    t = t >> 28u;
    ...
    t2 *= 2;
    t2 += (udpint)(spint)(lo + hi) * (udpint)0x17600;
    hi = (spint)(tt >> 28u);
    t += t2;
    spint v1 = (spint)t & mask;
    t = t >> 28u;
...
static void modsqrt_PM383_ct(const spint *x, const spint *h, spint *r) {
    ...
    spint b[14];
    spint v[14];
    spint z[14] = {0xeddd6da, 0x3bd5336, 0x732e0d4, 0xb000cb8, 0xa4da30,  0x12dde4b, 0x60e338b,
                   0x94425c2, 0x2471129, 0x15c9a42, 0x97cfdb0, 0x730f480, 0xdf95ff4, 0x24617};
    spint s[14];
    spint y[14];
    if (h == NULL) {
        modpro_PM383_ct(x, y);
