...
#define Wordlength_MFP248_ct 32
#define Nlimbs_MFP248_ct 9
#define Radix_MFP248_ct 29
...
static int modfsb_MFP248_ct(spint *n) {
    n[0] += (spint)1u;
    n[8] -= (spint)0x50000u;
    return flatten(n);
}
...
static void modadd_MFP248_ct(const spint *a, const spint *b, spint *n) {
    spint carry;
    n[0] = a[0] + b[0];
    n[1] = a[1] + b[1];
    n[2] = a[2] + b[2];
...
static void modsub_MFP248_ct(const spint *a, const spint *b, spint *n) {
    spint carry;
    n[0] = a[0] - b[0];
    n[1] = a[1] - b[1];
    n[2] = a[2] - b[2];
...
// Modular negation
static void modneg_MFP248_ct(const spint *b, spint *n) {
    spint carry;
    n[0] = (spint)0 - b[0];
    n[1] = (spint)0 - b[1];
    n[2] = (spint)0 - b[2];
...
static void modmul_MFP248_ct(const spint *a, const spint *b, spint *c) {
    dpint t = 0;
    spint p8 = 0x50000u;
...
static void modsqr_MFP248_ct(const spint *a, spint *c) {
    udpint tot;
    udpint t = 0;
    spint p8 = 0x50000u;
    spint q = ((spint)1 << 29u);  // q is unsaturated radix
...
// copy
static void modcpy_MFP248_ct(const spint *a, spint *c) {
    int i;
    for (i = 0; i < 9; i++) {
        c[i] = a[i];
    }
...
static void nres_MFP248_ct(const spint *m, spint *n) {
    const spint c[9] = {0xcf5c28fu, 0x6666666u,  0x13333333u, 0x19999999u, 0xcccccccu,
                        0x6666666u, 0x13333333u, 0x19999999u, 0x1ccccu};
    modmul_MFP248_ct(m, c, n);
}

// Convert n back to normal form, m=redc(n)
static void redc_MFP248_ct(const spint *n, spint *m) {
    int i;
    spint c[9];
    c[0] = 1;
    for (i = 1; i < 9; i++) {
    ...
    modmul_MFP248_ct(n, c, m);
    (void)modfsb_MFP248_ct(m);
}
...
static void modmli_MFP248_ct(const spint *a, int b, spint *c) {
    spint t[9];
    modint_MFP248_ct(b, t);
    modmul_MFP248_ct(a, t, c);
}
...
static int modqr_MFP248_ct(const spint *h, const spint *x) {
    ...
    return modis1_MFP248_ct(r) | modis0_MFP248_ct(x);
}

// conditional move g to f if d=1
// strongly recommend inlining be disabled using compiler specific syntax
static void modcmv_MFP248_ct(int b, const spint *g, volatile spint *f) {
    int i;
    spint c0, c1, s, t, aux;
    spint r = 0x5aa5a55au;
    c0 = (~b) & (r + 1);
...
// strongly recommend inlining be disabled using compiler specific syntax
static void modcsw_MFP248_ct(int b, volatile spint *g, volatile spint *f) {
    int i;
...
static void modshl_MFP248_ct(unsigned int n, spint *a) {
    int i;
    a[8] = ((a[8] << n)) | (a[7] >> (29u - n));
    for (i = 7; i > 0; i--) {
...
static int modshr_MFP248_ct(unsigned int n, spint *a) {
    int i;
...
static int modimp_MFP248_ct(const char *b, spint *a) {
    int i, res;
    for (i = 0; i < 9; i++) {
        a[i] = 0;
    }
