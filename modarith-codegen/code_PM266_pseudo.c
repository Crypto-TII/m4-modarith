...
static void modmli_PM266_ct(const spint *a, int b, spint *c) {
    ...
    udpint ut = (udpint)t;
    ut = (ut << 4) + (spint)(v9 >> 23u);
    v9 &= 0x7fffff;
    ut *= 0x3;
    s = v0 + ((spint)ut & mask);
    c[0] = (spint)(s & mask);
    carry = (s >> 27) + (spint)(ut >> 27);
    c[1] = v1 + carry;
    c[2] = v2;
    c[3] = v3;
    c[4] = v4;
...
static void modmul_PM266_ct(const spint *a, const spint *b, spint *c) {
    ...
    tt += (dpint)a[9] * (dpint)b[1];
    tt *= 0x30;
    t += tt;
    t += (dpint)a[0] * (dpint)b[0];
    spint v0 = (spint)t & mask;
    t = t >> 27u;
    tt = (dpint)a[2] * (dpint)b[9];
...
static void modsqrt_PM266_ct(const spint *x, const spint *h, spint *r) {
    ...
    spint b[10];
    spint v[10];
    spint z[10] = {0x1d89b35, 0x4b7561f, 0x74db88d, 0x540cef3, 0x39587a9,
                   0x13ec72c, 0xd16a3e,  0x74162d7, 0x2e254ef, 0x1c4d32};
    spint s[10];
    spint y[10];
    if (h == NULL) {
        modpro_PM266_ct(x, y);
